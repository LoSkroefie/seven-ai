...
Each server owns its concurrency state and lazily created agent. When an existing GUI/daemon agent is injected, the caller retains ownership. Clean shutdown stops accepting sockets, waits up to ten seconds for active handlers, closes the port, and shuts down only an API-owned idle agent. The listener enables address reuse specifically so a clean restart can reclaim the port despite prior connections in TCP `TIME_WAIT`; a simultaneously active listener is still rejected. GUI and daemon modes call that lifecycle explicitly. A port conflict makes `--api-only` exit nonzero with a visible startup error; GUI/daemon log the failure and continue their primary mode.
