...
| `integrations/calendar.py` | Calendar read/create | Portable local RFC5545 calendar tools | **Ported and superseded** with audited add/list/remove operations and no provider lock-in |
| `integrations/email_checker.py` | Mail checks | Credential-safe SMTP/IMAP tools | **Ported and superseded**; secrets remain environment-only and live mail stays disabled until explicitly configured |
...
| `integrations/pdf_generator.py` | PDF creation | Bounded PDF creation tool | **Ported and superseded** with declared ReportLab dependency and read-back tests |
