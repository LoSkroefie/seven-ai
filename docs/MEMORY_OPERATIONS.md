...
Seven's current SQLite schema is explicitly versioned with `PRAGMA user_version=1`.
...
The UTF-8 JSON export includes format/version metadata, Seven/schema versions, timestamp, source database SHA-256 and ordered records for conversation, facts, goals, tasks, notes, beliefs, working memory, skills, plans, embeddings, digests and preferences.
